...
Пользователь регистрируется в системе через OAuth2 провайдер Google. Единственный доступный способ входа - через Google аккаунт.

### Actors

- **Primary:** Пользователь (незарегистрированный)
- **Secondary:** Система OAuth2 (Google)

### Preconditions

- Пользователь не авторизован в системе
- Доступ в интернет
- Сервис Google доступен

### Postconditions

- Пользователь успешно зарегистрирован и авторизован
- Создан профиль пользователя в базе данных на основе данных из Google аккаунта
...
| ---------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **Happy Path**         | Успешная регистрация через Google         | 1. Пользователь нажимает "Войти через Google"<br>2. Система перенаправляет на страницу авторизации Google<br>3. Пользователь выбирает Google аккаунт и вводит пароль (или использует биометрию)<br>4. Google подтверждает авторизацию и возвращает токен с данными пользователя<br>5. Система создает профиль пользователя в базе данных<br>6. Пользователь перенаправлен на главную страницу с активной сессией | **Мгновенный доступ:** Регистрация и вход за 10-15 секунд без заполнения форм       |
| **Alternative Flow 1** | Пользователь уже зарегистрирован          | 1-4. (как в Happy Path)<br>5. Система обнаруживает существующий аккаунт по email из Google данных<br>6. Система автоматически авторизует пользователя<br>7. Показывается уведомление: "С возвращением! Вы вошли как [Имя пользователя]"                                                                                                                                                                          | **Удобство:** Автоматический вход без необходимости запоминать логины и пароли      |
| **Alternative Flow 2** | Использование нескольких Google аккаунтов | 1. Пользователь нажимает "Войти через Google"<br>2. Выбирает один из нескольких привязанных Google аккаунтов<br>3. Система создает новый профиль или использует существующий в зависимости от email<br>4. Пользователь авторизован с выбранным аккаунтом                                                                                                                                                         | **Гибкость:** Возможность использовать разные аккаунты для разных целей             |
| **Error Scenario 1**   | Сервис Google недоступен                  | 1. Пользователь нажимает "Войти через Google"<br>2. Система не может подключиться к Google OAuth API<br>3. Показывается ошибка: "Сервис Google временно недоступен. Проверьте интернет-соединение и попробуйте позже"<br>4. Кнопка "Повторить" становится активной через 30 секунд                                                                                                                               | **Надежность:** Понятная информация о проблеме и возможность повторить попытку      |
| **Error Scenario 2**   | Отмена авторизации пользователем          | 1-2. (как в Happy Path)<br>3. Пользователь нажимает "Отмена" на странице выбора Google аккаунта<br>4. Система возвращает на страницу входа<br>5. Показывается сообщение: "Авторизация отменена"                                                                                                                                                                                                                  | **Контроль:** Пользователь может легко передумать без последствий                   |
| **Error Scenario 3**   | Неверные данные от Google                 | 1-4. (как в Happy Path)<br>5. Google возвращает ошибку или некорректные данные пользователя<br>6. Система показывает: "Произошла ошибка при получении данных. Попробуйте снова"<br>7. После 3 неудачных попыток показывается: "Обратитесь в поддержку"                                                                                                                                                           | **Безопасность:** Защита от некорректных данных и понятные инструкции при проблемах |
