...
graph TD
    ...
    %% Контейнеры
    FE[🖥️ Веб-интерфейс (Frontend)]
    BE[⚙️ Серверное приложение (Backend)]
    DB[💾 База данных (Database)]
    AD[🧑‍💼 Административная панель (Admin Dashboard)]

    %% Взаимодействия пользователя с системой
    U -->|Использует функционал: регистрация, создание планов, просмотр рецептов, экспорт списка| FE
    FE -->|REST API: отправка запросов и получение данных| BE
    BE -->|SQL / ORM: хранение и получение данных| DB

    %% Взаимодействие администратора
    Admin -->|Управление рецептами и анализ статистики| AD
    AD -->|REST API / SQL| BE
    BE -->|SQL / ORM| DB
